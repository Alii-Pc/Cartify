...
export function CategoryDetailsClient({
  ...
  const [products, setProducts] = useState<SafeProduct[]>(initialProducts);
  const [category, setCategory] = useState<SafeCategory | null>(initialCategory);
  ...
  useEffect(() => {
    if (initialProducts.length > 0 && sortParam === "newest" && category) {
      return;
    }
    ...
    async function fetchCategoryProducts() {
      ...
      try {
        ...
        const prodRes = await fetch(
          `/api/products?category=${slug}&sort=${sortParam}&limit=50`
        );
  ...
  }, [slug, sortParam, category, initialProducts.length]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
      ...
      <nav
        ...
        className="mb-6 flex items-center gap-2 text-xs font-medium text-charcoal-700/60"
      >
